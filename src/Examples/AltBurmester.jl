...
function AltsProblem()
    ...
    @var G1r,G2r,G1i,G2i,z1r,z2r,z1i,z2i,dx[1:9],dy[1:9],tr[1:9],ti[1:9]
    ...
    eq2 = []
    for i in 2:9
	   push!(eq2,(tr[i]+im*ti[i])*(tr[i]-im*ti[i])-1)
    end
    ...
    for i in 2:9
        push!(eq5,(tr[i]+im*ti[i])*z1+d[i]-g1)
        push!(eq6,(tr[i]-im*ti[i])*Z1+D[i]-G1)
        push!(eq8,(tr[i]+im*ti[i])*z2+d[i]-g2)
        push!(eq9,(tr[i]-im*ti[i])*Z2+D[i]-G2)
    end
    ...
    eq10=[]
    for i in 2:9
        push!(eq7,eq5[i]*eq6[i]-eq5[1]*eq6[1])
        push!(eq10,eq8[i]*eq9[i]-eq8[1]*eq9[1])
    end
    ...
    vars=Vector{Variable}([G1r,G1i,G2r,G2i,z1r,z2r,z1i,z2i])
