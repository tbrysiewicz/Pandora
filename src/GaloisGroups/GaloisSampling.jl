...
function monodromy_element(E::EnumerativeProblem)
    if is_populated(E)==false
        populate_base_fibre(E)
    end
    γ = [base_parameters(E)] #starting at BaseParam
    for i in 1:3
        push!(γ,last(γ)+randn(ComplexF64,n_parameters(E))) #with 3 other parameter values
    end
    push!(γ,base_parameters(E)) #and ending again at BaseParam
    return monodromy_element(E,γ)
end

function sample_monodromy_elements(E::EnumerativeProblem,n::Int64)
    println("Sampling ",n," elements from the monodromy group of ",E)
    S = [monodromy_element(E) for i in 1:n]
